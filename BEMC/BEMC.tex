...
%Fortunately, for related functions known as the probabilists' Hermite polynomials, $\int_{a}^b He_n(x)e^{\frac{-x^2}{2}}dx=-He_{n+1}(x)e^{\frac{-x^2}{2}}$ \cite{hermite_integrate}.
%terms of the form $x^k\exp(\frac{-x^2}{2})$. , so $\int_{x_1}^{x_2} x^k\exp(\frac{-x^2}{2})$ is $\frac{ \Gamma(k+1)}{(\frac{1}{2})^{\frac{k+1}{2}}}$ times \texttt{pnaka(x$_2$, shape=(k+1)/2, rate=k+1)-pnaka(x$_1$, shape=(k+1)/2, rate=k+1)}. 
