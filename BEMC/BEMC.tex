...
It is shown in \cite{bacharoglou2010approximation} that any continuous and compactly-supported probability density on $\mathbb{R}^D$ can be approximated arbitrarily well by a finite mixture of Gaussians. 

Consider a random variable $X$ drawn from the target density $\pi$, and consider a random variable $Y$ obtained by applyng a single iteration of Metropolis-Hastings to $X$. $Y$ has marginal density $\pi$. The object of inference in BEMC, $K$, equals the density of $Y|X$. 
