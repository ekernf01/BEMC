...
prepare_basis = function(dimension = 1, 
  ...
  # This code find the 4 nearest neighbors for each element.
...
bemc = function(basis, sampler, dimension = 1, NUM_SAMPLES = 10000){
  ...
  basis$sampler_action = basis$sampler_action -
    diag_shrink * diag(diag(basis$sampler_action))
...
plot(basis$means, diag(basis$sampler_action))
lines(my_grid, target(my_grid))
plot(basis$target_coeffs)
```